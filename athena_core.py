...
class AthenaClient:
    """Handles AI query parsing specifically for client briefs using Ollama."""
    ...
    def create_client_brief_prompt(self, client_brief: str) -> str:
        """Create specialized prompt for parsing client briefs."""
        return f"""You are Athena, an AI fashion assistant that extracts structured filters from client briefs.
...
Output:"""
    ...
    def parse_client_brief(self, client_brief: str) -> Optional[Dict[str, Any]]:
        """Parse client brief using Ollama and return structured filters."""
        try:
            prompt = self.create_client_brief_prompt(client_brief)
...
class EmailGenerator:
    """Generates professional email pitches using Ollama."""
    ...
    def create_email_prompt(self, client_brief: str, selected_models: List[Dict[str, Any]]) -> str:
        """Create prompt for generating professional email pitch."""
        ...
        return f"""You are Athena, an AI assistant that drafts professional, brand-aligned emails for fashion agencies.

Given a client brief and selected model profiles, generate a short, elegant email pitch suitable for brand communication.
...
Keep it concise, professional, and highlight how the selected models fit the client's requirements."""
    ...
    def generate_email_pitch(self, client_brief: str, selected_models: List[Dict[str, Any]],
        ...
        """Generate email pitch using Ollama."""
        try:
            prompt = self.create_email_prompt(client_brief, selected_models)
